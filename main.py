...
if __name__ == '__main__':
    ...
    env_id = envs[0]    # select environment
    ...
    if algo == 'TD3':
        ...
        agent = Agent_sac(env_id, env, input_dims, num_actions, lr_alpha=3e-4, lr_beta=3e-4, lr_kappa=3e-4, 
                ...
                batch_size=256, reparam_noise=1e-6, reward_scale=scale, loss_type =loss_fn, cauchy_scale=0.420, algo_name=algo)

    n_rounds = 1    # number of trials
    n_steps = 3e6    # maximum cumulative steps per round
    n_games = 300    # maximum number of episodes per round
    ...
    for itr in range(n_rounds):
        ...
        for i in range(n_games):
            ...
            print('epis/step/tot {}/{}/{}: score {:1.1f}, trail {:1.1f}, C/A loss {:1.1f}/{:1.1f}'
                .format(i+1, step, cum_steps, score, trail_score, q1_loss+q2_loss, actor_loss))
        ...
        filename_npy_logtemp = directory + str(itr+1)+'_logtemp'+'.npy'
