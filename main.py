...
for loss_fn in critic_loss:
    ...
    inputs = {
            ...
            'policy_noise': 0.1,        # Gaussian noise added to next agent action exploration
            ...
            'loss_fn': loss_fn,         # loss 'Cauchy', 'CIM', 'HSC', 'Huber', 'MAE', 'MSE', 'MSE2', 'MSE4', 'MSE6', 'TCauchy'
    ...
    if inputs['algo'] == 'TD3':
        batch_size = 100
        agent = Agent_td3(env_id, env, lr_alpha=0.001, lr_beta=0.001, tau=0.005, layer1_dim=400, layer2_dim=300, 
                          cauchy_scale_1=0.420, cauchy_scale_2=0.420, gamma=inputs['discount'], erg=inputs['ergodicity'], 
                          loss_type=inputs['loss_fn'], max_size=inputs['buffer'], algo_name=inputs['algo'], 
                          actor_update_interval=2, batch_size=batch_size, warmup=inputs['random'], 
        ...
        batch_size = 256
        agent = Agent_sac(env_id, env, lr_alpha=3e-4, lr_beta=3e-4, lr_kappa=3e-4, tau=0.005, 
                gamma=inputs['discount'], actor_update_interval=1, max_size=inputs['buffer'], layer1_dim=256, 
                layer2_dim=256, batch_size=batch_size, reparam_noise=1e-6, reward_scale=inputs['r_scale'], 
                stoch=inputs['s_dist'], loss_type =inputs['loss_fn'], cauchy_scale_1=0.420, cauchy_scale_2=0.420, 
    ...
    for round in range(inputs['n_trials']):
        ...
        while cum_steps < int(inputs['n_cumsteps']):
            ...
            while not done:
                ...
                agent.store_transistion(state, action, reward, next_state, done)    # no current transistion sampling
            ...
            trial_log[round, episode, 0], trial_log[round, episode, 1] =  np.array(end_time-start_time), np.array(score)
            trial_log[round, episode, 2], trial_log[round, episode, 3:6] = np.array(step), np.array(loss)
            trial_log[round, episode, 6], trial_log[round, episode, 7:] = np.array(logtemp), np.array(loss_params)
            ...
            print('{} {} {:1.0f}/s ep/st/cst {}/{}/{}: r {:1.0f}, r{} {:1.0f}, C/A loss {:1.1f}/{:1.1f}'
                .format(fin.strftime('%d %H:%M:%S'), loss_fn, step/trial_log[round, episode, 0], episode, step, 
